...
\begin{neuralnetwork}[height=1,maintitleheight=1cm,layertitleheight=2.5cm, layerspacing=50]
  ...
  \newcommand{\textout}[2]{\ifnum #2=1 $z_{2,j}$ \else $z_{2,#2}$ \fi}
  
  \newcommand{\w}[4] {\ifnum #2=4 \ifnum #4=1 $w_{k,j}^{(2)}$ \else $w_{k,#4}^{(2)}$ \fi \else \ifnum #4=2 $w_{#2,j}^{(2)}$ \else $w_{#2,#4}^{(2)}$ \fi \fi}
  ...
  % draw dots
    ...
    \draw [->,>=stealth,gray] (L2-1) -- node [black,midway,above] {$\hat{y}_1=g(z_{2,j})$} +(2,0);
